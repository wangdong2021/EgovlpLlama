...
class CrossAttentionFromVarAttention(nn.Module):
    def __init__(self, dim, text_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.key = nn.Linear(text_dim, dim, bias=qkv_bias)
        self.value = nn.Linear(text_dim, dim, bias=qkv_bias)

        self.proj = nn.Linear(dim, dim)
        if initialize == 'zeros':
            self.query.weight.data.fill_(0)
            self.query.bias.data.fill_(0)
            self.key.weight.data.fill_(0)
            self.key.bias.data.fill_(0)
            self.value.weight.data.fill_(0)
    ...
    def forward(self, x, text_features, text_attention_mask, einops_from, einops_to, **einops_dims):
        h = self.num_heads
        # project x to q, k, v vaalues
        # q: (B, N, D)
        # k: 
...
class CrossAttentionFromBertAttention(nn.Module):
    def __init__(self, dim, text_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 initialize='random'):
        super().__init__()
        self.num_heads = num_heads
    ...
    def forward(self, x, text_features: torch.Tensor, text_attention_mask:torch.Tensor=None):
        """
        ...
        if text_attention_mask is not None:
            text_attention_mask = self.get_text_attention_mask(text_attention_mask, attention_scores.dtype)
            attention_scores = attention_scores + text_attention_mask
    ...
    def get_text_attention_mask(self, text_attention_mask: torch.Tensor, dtype: torch.dtype):
        """
        text_attention_mask: (batch_size, seq_len)
        """
        text_attention_mask = text_attention_mask.unsqueeze(1).repeat(1, self.num_heads, 1).view(text_attention_mask.shape[0] * self.num_heads, 1, -1)
        if dtype == torch.float16:
            return (1.0 - text_attention_mask.half()) * -1e4
        else:
            return (1.0 - text_attention_mask) * -1e9

class SpaceTimeBlock(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, time_init='zeros',
                 attention_style='frozen-in-time', layer_scale_init_values=None):
        super().__init__()
        ...
        # self.cross_attention = CrossAttentionFromVarAttention(
            # dim, text_dim=768, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            # initialize=time_init)
        
        self.cross_attention = CrossAttentionFromBertAttention(
            dim, text_dim=768, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            initialize=time_init)
        ...
        self.norm2_ego = norm_layer(dim)
        self.norm2_third = norm_layer(dim)
        ...
        self.mlp_ego = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.mlp_third = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, einops_from_space, einops_to_space, einops_from_time, einops_to_time,
                time_n, space_f, is_ego=True, is_cross=False, text_features=None, text_attention_mask=None):
        ...
        if is_cross:
            space_output = self.cross_attention(
                self.norm_cross(space_residual), 
                text_features=text_features,
                text_attention_mask=text_attention_mask,
            )
            space_residual = x + self.drop_path(space_residual)
        if is_ego:
            x = space_residual + self.drop_path(self.mlp_ego(self.norm2_ego(space_residual)))
        else:
            x = space_residual + self.drop_path(self.mlp_third(self.norm2_third(space_residual)))
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.itm_head = nn.Linear(self.num_features, 2) if num_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x, is_ego=True, is_cross=False, text_features=None, text_attention_mask=None, select_layer: int=-1):
        ...
        for blk in self.blocks:
            x = blk(x, self.einops_from_space, self.einops_to_space, self.einops_from_time,
                    self.einops_to_time,
                    time_n=n, space_f=f, 
                    is_ego=is_ego, is_cross=is_cross,
                    text_features=text_features, text_attention_mask=text_attention_mask)
            hidden_states.append(x)
        return hidden_states[select_layer]

    def forward(self, x, is_ego=True, is_cross=False, text_features=None, text_attention_mask=None):
        x = self.forward_features(x, is_ego, is_cross, text_features, text_attention_mask)
        x = self.norm(x)[:, 0]
        x = self.pre_logits(x)
        if not is_cross:
            x = self.head(x)
